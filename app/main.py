...
#         # should_block = risk_level == RiskLevel.CRITICAL or risk_level == RiskLevel.HIGH
...
app = FastAPI(
    title=settings.APP_NAME,
    version=settings.VERSION,
    description="AML Transaction Screening API - Geographic & Security Risk",
    docs_url=f"{settings.API_PREFIX}/docs",
    redoc_url=f"{settings.API_PREFIX}/redoc",
)
...
async def root():
    """Root endpoint."""
    return {"service": settings.APP_NAME, "status": "operational"}
...
@app.post(
    ...
    summary="Screen Transaction",
    description="Screen a transaction for geographic and security risks (VPN, Tor, etc.)"
)
async def screen_transaction(request: ScreeningRequest):
    try:
        logger.info(f"Screening transaction: {request.transaction_id} | IP: {request.ip_address}")
        
        # 1. Perform Enhanced GeoIP & Security Lookup
        geoip_result = await geoip_service.lookup(request.ip_address)
        ...
        if not geoip_result.success:
            ...
            return ScreeningResponse(
                ...
                triggered_rules=[],
                recommendation=f"WARNING: GeoIP service unavailable. Manual review recommended. Error: {geoip_result.error}",
                timestamp=datetime.utcnow().isoformat()
            )
        
        logger.info(f"GeoIP Success: {request.ip_address} -> {geoip_result.country_code} (VPN={geoip_result.is_vpn})")
        
        # 2. Assess Risk (NOW INCLUDES SECURITY FLAGS)
        # We pass the new flags from geoip_result into the risk engine
        risk_score, risk_level, triggered_rules, recommendation = await risk_engine.assess_risk(
            user_country=request.user_country,
            ip_country=geoip_result.country_code,
            geoip_confidence=geoip_result.confidence,
            # --- PASS NEW FLAGS HERE ---
            is_vpn=geoip_result.is_vpn,
            is_tor=geoip_result.is_tor,
            is_proxy=geoip_result.is_proxy
            # ---------------------------
        )
        ...
        return ScreeningResponse(
            ...
            confidence=geoip_result.confidence,
            user_country=request.user_country,
            detected_country=geoip_result.country_code,
            countries_match=(request.user_country == geoip_result.country_code),
            security=SecurityDetails(
                is_vpn=geoip_result.is_vpn,
                is_proxy=geoip_result.is_proxy,
                is_tor=geoip_result.is_tor,
                is_relay=geoip_result.is_relay
            ),
            triggered_rules=triggered_rules,
            recommendation=recommendation,
            timestamp=datetime.utcnow().isoformat()
